...
from assistant.application.generate_response import get_response
from assistant.application.agents.state import state_to_str
...
def evaluate_agent(
    ...
    assert settings.COMET_API_KEY, (
        "COMET_API_KEY is not set. Needed to track the experiment with Opik."
    )
    ...
    experiment_config = {
        "model_id": settings.EVALUATION_LLM,
